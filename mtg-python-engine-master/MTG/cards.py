...
def add_activated_ability(cardname, cost, effect, target_criterias=None, prompts=None):
    ...
    card.activated_abilities.append(
        (costs, effect, target_criterias, prompts, is_mana_ability)
    )
...
def add_aura_effect(cardname, effects, target_criterias=["creature"]):
    ...
    add_play_func_with_targets(
        cardname, lambda self, targets, l: permanent.make_aura(self, targets[0])
    )
...
def add_trigger(
    ...
    card.triggers[condition].append(
        (effect, requirements, target_criterias, target_prompts, intervening_if)
    )
...
def parse_card_from_lines(lines, log=None):
    ...
    for line in lines:
        ...
        if stage == "effects":  # read in card effects
            ...
            if ind_lv == 2:
                ...
                )  # remove quotes, convert to string
            ...
            if ind_lv == 2:  # new trigger
                ...
                if line == "Conditioned On:":  # whenever ...
                    ...
                    line == "If:"
                ):  # ... if ...  -- MUST COME AFTER 'Conditioned On' if both present
            ...
            if ind_lv == 2:
                ...
                static_effects[-1].append(
                    line
                )  # effect_name, effect_value, toggle_func
    ...
    if _triggers:
        for trig in _triggers:
            ...
            str_to_exe += (
                "add_trigger(%s, triggers.triggerConditions[%r], %r, %s, %s, intervening_if=%s)\n"
                % (name, *trig)
            )
    ...
    if effects:
        if not targets:
            ...
            effects = (
                "lambda self, targets, is_legal_target: [" + ", ".join(effects) + "]"
            )
