...
class Play(gameobject.GameObject):
    ...
    def __init__(
        ...
        super().__init__(
            zone=controller.stack if controller else None,
    ...
    def apply(self):
        ...
        elif self.targets_chosen and not any(
            [
                c(self, t) and t.timestamp == time
                for c, t, time in zip(
                    self.target_criterias, self.targets_chosen, self.target_timestamps
                )
            ]
        ):
