    ...
    for card in card_list:
        ...
        try:
            ...
            characteristics["color"] = (
                card["colorIdentity"] if "colorIdentity" in card else ""
            )
            ...
            if "supertypes" in card:
                supertype = (
                    "["
                    + ", ".join(
                        ["cardtype.SuperType." + i.upper() for i in card["supertypes"]]
                    )
                    + "]"
                )
            ...
            types = (
                "["
                + ", ".join(["cardtype.CardType." + i.upper() for i in card["types"]])
                + "]"
            )

            if "Creature" in card["types"]:
                characteristics["power"], characteristics["toughness"] = (
                    star_or_int(card["power"]),
                    star_or_int(card["toughness"]),
                )
