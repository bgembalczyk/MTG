...
def run():
    ...
    for card in card_list:
        ...
        types = []
        try:
            ID = "c" + re.search("(?<=multiverseid=)[0-9]+", card.set["picURL"]).group(
                0
            )
            ...
            types = (
                "["
                + ", ".join(["cardtype.CardType[%r]" % i.upper() for i in _type])
                + "]"
            )

            if "Creature" in _type:
                try:
                    characteristics["power"], characteristics["toughness"] = map(
                        int, card.find("pt").text.split("/")
                    )
                except ValueError:
                    pass
            # static abilities
